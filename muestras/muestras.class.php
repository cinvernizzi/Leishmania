...
class Muestras {
    ...
    public function getPendientesPacientes() : array {
        ...
        $consulta = "SELECT leishmania.v_pacientes.fecha AS fecha,
                            ...
                            leishmania.v_muestras.fecha AS fecha_muestra,
                            leishmania.v_pacientes.usuario AS usuario 
                     FROM leishmania.v_pacientes INNER JOIN leishmania.v_muestras ON leishmania.v_pacientes.id = leishmania.v_muestras.idpaciente
                     WHERE ISNULL(leishmania.v_muestras.resultado) AND
                           NOT ISNULL(leishmania.v_muestras.material)
                     ORDER BY STR_TO_DATE(leishmania.v_muestras.fecha, '%d/%m/%Y'),
    ...
    /**
     * Método que obtiene el número de muestras de los pacientes que 
     * aún no tienen resultados cargados
     * @author Claudio Invernizzi <cinvernizzi@dsgestion.site>
    ...
    /**
     * Método que recibe el desplazamiento a partir del primer 
     * registro y el número de registros a retornar y retorna
     * el vector con los pacientes que tienen muestras pero 
     * que aún no se ha cargado el resultado, utilizado en 
     * la paginación de la grilla de los reportes
     * @author Claudio Invernizzi <cinvernizzi@dsgestion.site>
    ...
    /**
     * Método que retorna el número de pacientes con resultados 
     * cargados que aún no han sido notificados al sisa
     * @author Claudio Invernizzi <cinvernizzi@dsgestion.site>
     * @return int cantidad de registros
     */
    public function numeroSinNotificar() : int {
    ...
    /**
     * Método que recibe como parámetro el desplazamiento del primer 
     * registro y el número de registros a retornar y devuelve el 
     * vector con los pacientes que tienen resultados cargados y 
     * que aún no fueron notificados al sisa, utilizado en la 
     * paginación de los reportes 
     * @author Claudio Invernizzi <cinvernizzi@dsgestion.site>
     * @param int $offset - desplazamiento del primer registro
     * @param int $registros - número de registros a retornar
     * @return array - vector con los registros
     */
    public function sinNotificarPaginados(int $offset, int $registros) : array {
